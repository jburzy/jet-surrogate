...
def skim_truth(part: ak.Array) -> tuple[np.ndarray, np.ndarray]:
    ...
    pcols, part_jet = F.particle_columns(part, part_jet, truth_jets, pv_z=F.primary_vertex_z(part))
...
def skim_events(ev: ak.Array) -> dict:
    ...
    # everything longitudinal is measured from the primary vertex: the one
    # Delphes wrote if the sample has pile-up, else estimated from the record
    pv = np.asarray(ev.pv_z, dtype=np.float32) if "pv_z" in ev.fields else None
    if pv is None or not np.any(pv != 0.0):
        pv = F.primary_vertex_z(ev.part)

    # tracks -> reco jets
    trk_jet = associate(ev.trk, reco)
    reco_jets = jet_table(reco, r2t, np.zeros(len(reco), np.int32))
    tcols, trk_jet = F.track_columns(ev.trk, trk_jet, reco_jets, pv_z=pv)
    ...
    pcols, part_jet = F.particle_columns(ev.part, part_jet, truth_jets, pv_z=pv)
    truth_jets["n_assoc"] = np.bincount(part_jet[part_jet >= 0], minlength=len(truth))
    truth_parts = F.pad_groups(part_jet, len(truth), pcols["pt"], pcols, F.MAX_PART,
                               F.PART_FLOATS, F.PART_CATS)
